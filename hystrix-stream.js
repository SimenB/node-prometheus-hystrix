...
const requestCount = new Gauge('hystrix_requests_total', 'Total number of requests made to the breaker', [
  'breakerName',
]);
...
const rollingCountExceptionsThrown = new Gauge(
  'hystrix_exceptions_thrown_total',
  'Total number of times an exception has been thrown.',
  ['breakerName']
);
const rollingCountFailure = new Gauge('hystrix_failures_total', 'Total number of times a call has failed.', [
  'breakerName',
]);
const rollingCountFallbackFailure = new Gauge(
  'hystrix_fallback_failures_total',
  'Total number of times a fallback has failed.',
  ['breakerName']
);
const rollingCountFallbackRejection = new Gauge(
  'hystrix_fallback_rejections_total',
  'Total number of times a fallback has rejected.',
  ['breakerName']
);
const rollingCountFallbackSuccess = new Gauge(
  'hystrix_fallback_successes_total',
  'Total number of times a fallback has been successful.',
  ['breakerName']
);
...
const rollingCountShortCircuited = new Gauge(
  'hystrix_short_circuits_total',
  'Total number of times a request short circuited.',
  ['breakerName']
);
const rollingCountSuccess = new Gauge('hystrix_successes_total', 'Total number of times a request succeeded.', [
  'breakerName',
]);
...
const rollingCountTimeout = new Gauge('hystrix_timeouts_total', 'Total number of times a request timed out.', [
  'breakerName',
]);
...
const latencyExecuteMean = new Gauge('hystrix_latency_execute_mean', 'Mean latency of all executions.', [
  'breakerName',
]);
const latencyTotalMean = new Gauge('hystrix_latency_total_mean', 'Mean latency of all requests.', ['breakerName']);

export default stream => fromStream(stream).map(data => data.replace(/^data: /, '')).map(JSON.parse).subscribe(({
  name,
  ...data
}) => {
